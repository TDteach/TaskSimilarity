...
# https://github.com/naiyeleo/ABS/blob/master/TrojAI_competition/round1/abs_pytorch_round1.py
            ...
            for nt in range(nbatches):
                ...
                for neuron in range(sample_batch_size):
                    ...
                    for img_i in range(batch_size):
                        ...
                        ps_key = (img_name, '{0}_{1}'.format(children[layer_i].__class__.__name__, layer_i),
                                  neuron + nt * sample_batch_size)
...
def loss_fn(inner_outputs_b, inner_outputs_a, logits, con_mask, neuron, tlabel, acc, e, re_epochs):
    ...
    mask_add_loss = torch.where(mask_cond1,
                                torch.where(mask_cond2, 2 * re_mask_weight * mask_loss, 1 * re_mask_weight * mask_loss),
                                0.01 * mask_loss)
    loss += mask_add_loss
...
def reverse_engineer(model_type, model, children, oimages, olabels, weights_file, Troj_Layer, Troj_Neuron, Troj_Label,
                     Troj_size, re_epochs, preprocess_func=None):
    before_block = []
    ...
    if model_type == 'ResNet':
        ...
        # TDteach
        # get last bn layer for ResNet18
        if len(children_modules) > 5:
            bn3_module = children_modules[5]
        else:
            bn3_module = children_modules[3]  # bn2
    ...
    for e in range(re_epochs):
        ...
        for i in range(len(images) // re_batch_size):
            ...
            if preprocess_func is not None:
                in_data = preprocess_func(in_data)
            ...
            if model_type == 'ResNet':
                ...
                if children[Troj_Layer].__class__.__name__ == 'InceptionE':
                    ...
                    inner_outputs_b = torch.cat(
                        [after_bn1_t, after_bn2_t, after_bn3_t, after_bn4_t, after_bn5_t, after_bn6_t], 1)
            ...
            loss, vloss1, vloss2, vloss3, relu_loss1, relu_loss2, mask_loss, mask_nz, mask_add_loss, logits_loss \
                = loss_fn(inner_outputs_b, inner_outputs_a, logits, use_mask, Troj_Neuron, int(Troj_Label), facc, e,
                          re_epochs)
            if e > 0:
        ...
        if e % 10 == 0:
            print(e, 'loss', loss.cpu().detach().numpy(), 'acc {:.4f}'.format(facc), 'target label', int(Troj_Label),
                  int(Troj_Label2), 'logits_loss', logits_loss.cpu().detach().numpy(), \
                  'vloss1', vloss1.cpu().detach().numpy(), 'vloss2', vloss2.cpu().detach().numpy(), \
                  'relu_loss1', relu_loss1.cpu().detach().numpy(), 'max relu_loss1',
                  np.amax(inner_outputs_a.cpu().detach().numpy()), \
                  'relu_loss2', relu_loss2.cpu().detach().numpy(), \
                  'mask_loss', mask_loss.cpu().detach().numpy(), 'mask_nz', mask_nz.cpu().detach().numpy(),
                  'mask_add_loss', mask_add_loss.cpu().detach().numpy())
...
def re_mask(model_type, model, neuron_dict, children, images, labels, scratch_dirpath, re_epochs, preprocess_func=None):
    validated_results = []
    for key in sorted(neuron_dict.keys()):
        ...
        for task in neuron_dict[key]:
            ...
            RE_img = os.path.join(scratch_dirpath, 'imgs',
                                  '{0}_model_{1}_{2}_{3}_{4}.png'.format(weights_file.split('/')[-2], Troj_Layer,
                                                                         Troj_Neuron, Troj_size, samp_label))
            RE_mask = os.path.join(scratch_dirpath, 'masks',
                                   '{0}_model_{1}_{2}_{3}_{4}.pkl'.format(weights_file.split('/')[-2], Troj_Layer,
                                                                          Troj_Neuron, Troj_size, samp_label))
            RE_delta = os.path.join(scratch_dirpath, 'deltas',
                                    '{0}_model_{1}_{2}_{3}_{4}.pkl'.format(weights_file.split('/')[-2], Troj_Layer,
                                                                           Troj_Neuron, Troj_size, samp_label))
            ...
            for i in range(mask_multi_start):
                acc, rimg, rdelta, rmask, optz_label = reverse_engineer(model_type, model, children, images, labels,
                                                                        weights_file, Troj_Layer, Troj_Neuron,
                                                                        samp_label, Troj_size, re_epochs, preprocess_func=preprocess_func)
...
def filter_stamp(n_img, trigger):
    ...
    tdelta = tf.placeholder(tf.float32, shape=(window_size, 3))
    ...
    i_image = tf.reshape(tf.matmul(tf.reshape(tf.concat([ti_image, imax, imin, iavg], axis=3), (-1, 12)), tdelta),
                         [-1, h, w, 3])
...
def test(model, model_type, test_xs, result, scratch_dirpath, mode='mask', preprocess_func=None):
    re_batch_size = config['re_batch_size']
    if model_type == 'DenseNet':
        re_batch_size = max(re_batch_size // 3, 1)
    if model_type == 'ResNet':
    ...
    for i in range(len(rt_images) // re_batch_size):
        ...
        #TDteach
        if preprocess_func is not None:
            batch_data = preprocess_func(batch_data)
...
from train_cifar10 import build_model, load_model, prepare_dataset_without_normalization, prepare_dataset
from test import get_test_preprocess_func


def main(model_filepath, result_filepath='./output', scratch_dirpath='./scratch', examples_dirpath=None, example_img_format='png', preprocess_func=None):
    start = time.time()

    # TDteach
    trainset, testset = prepare_dataset_without_normalization()
    ...
    for inputs, labels in testloader:
        imgs.append(inputs.numpy())
        labs.append(labels.numpy())
    fxs = np.concatenate(imgs)
    fys = np.concatenate(labs)
    ...
    results = re_mask(model_type, model, neuron_dict, children, xs, ys, scratch_dirpath, re_epochs, preprocess_func=preprocess_func)
    reasr_info = []
    reasrs = []
    if len(results) > 0:
        reasrs = []
        for result in results:
            reasr = test(model, model_type, test_xs, result, scratch_dirpath, result, preprocess_func=preprocess_func)
            reasrs.append(reasr)
            adv, rdelta, rmask, optz_label, RE_img, RE_mask, RE_delta, samp_label, acc = result
            rmask = rmask * rmask > mask_epsilon
            reasr_info.append(
                [reasr, 'mask', str(optz_label), str(samp_label), RE_img, RE_mask, RE_delta, np.sum(rmask), acc])
    ...
    print('{0} {1} {2} {3} {4} {5} {6} {7} {8}'.format( \
        model_filepath, model_type, 'mask', freasr, 'sampling val', sampling_val, 'time', sample_end - start,
                                                                                          optm_end - sample_end, ))
    if max_reasr >= 0.88:
        output = 1 - 1e-1
    else:
